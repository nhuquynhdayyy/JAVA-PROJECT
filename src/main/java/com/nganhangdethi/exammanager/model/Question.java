...
public class Question implements Cloneable {
	...
	// Phương thức để lấy text của đáp án đúng dựa trên correctAnswerKey hoặc
	// options
	...
	public String toString() {
		return questionID + ": "
				+ (questionText != null ? questionText.substring(0, Math.min(questionText.length(), 50)) : "N/A")
				+ "...";
	}

	// --- THÊM PHƯƠNG THỨC CLONE ---
	@Override
	public Question clone() throws CloneNotSupportedException { // <<<< ĐỔI TÊN VÀ KIỂU TRẢ VỀ
		Question cloned = (Question) super.clone();

		if (this.options != null) {
			cloned.options = new ArrayList<>();
			for (QuestionOption option : this.options) {
				cloned.options.add(option.clone()); // Gọi QuestionOption.clone()
			}
		}
		...
		if (this.updatedAt != null) {
			cloned.updatedAt = (Date) this.updatedAt.clone();
		}
